...
ggplot(ppg, aes(x=condition, y=rmssd, fill=condition))+
  ...
  theme_bw()+ 
  xlab("Route") + ylab("Heart Rate Variability (HRV)")

t.test(ppg2$rmssd~ppg2$condition)

ppg2 %>% cohens_d(rmssd ~ condition, var.equal = TRUE)

m<-lme(rmssd ~ condition , random = ~1|participantId,control=ctrl, data=ppg2, method="ML")
summary(m)

r2beta(m, partial = 'TRUE')
